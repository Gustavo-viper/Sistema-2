...
function renderBudgetRequests() {
  ...
  panel.innerHTML = `<h3>Solicitações de orçamento dos clientes</h3>${budgetRequestsCache.map(r => {
    const client = clientsCache.find(c => String(c.auth_user_id || '') === String(r.user_id || ''));
    const status = r.status || 'pending';
    return `
    <article class="budget-request-item">
      <strong>${escapeHTML(client?.name || r.user_name || 'Cliente não identificado')}</strong>
      <h4>${escapeHTML(r.device_type || 'Aparelho')} - ${escapeHTML(r.device_model || 'Modelo não informado')}</h4>
      <p><b>Problema:</b> ${escapeHTML(r.problem_type || 'Não informado')}</p>
      <p><b>Observações:</b> ${escapeHTML(r.observations || 'Sem observações')}</p>
      <small>Enviado em ${escapeHTML(formatDateTime(r.created_at))}</small>
      <div class="budget-request-controls">
        <label>Status
          <select id="request-status-${escapeHTML(r.id)}">
            ${['pending','reviewing','quoted','approved','rejected','cancelled'].map(option => `<option value="${option}" ${option === status ? 'selected' : ''}>${escapeHTML({pending:'Aguardando análise',reviewing:'Em análise',quoted:'Orçamento respondido',approved:'Aprovado',rejected:'Recusado',cancelled:'Cancelado'}[option])}</option>`).join('')}
          </select>
        </label>
        <label>Valor do orçamento (R$)
          <input id="request-value-${escapeHTML(r.id)}" type="number" min="0" step="0.01" value="${r.quoted_value ?? ''}" placeholder="Opcional">
        </label>
        <label>Resposta para o cliente
          <textarea id="request-response-${escapeHTML(r.id)}" rows="2" placeholder="Digite uma resposta">${escapeHTML(r.admin_response || '')}</textarea>
        </label>
        <button type="button" class="btn btn-primary btn-sm" onclick="updateBudgetRequest('${escapeHTML(r.id)}')">Salvar resposta</button>
      </div>
    </article>`;
  }).join('')}`;
}

async function updateBudgetRequest(requestId) {
  const supabase = getSupabase();
  if (!supabase || !requestId) return;

  const status = getElement(`request-status-${requestId}`)?.value || 'pending';
  const valueText = getElement(`request-value-${requestId}`)?.value.trim() || '';
  const response = getElement(`request-response-${requestId}`)?.value.trim() || '';
  const quotedValue = valueText === '' ? null : Number(valueText);

  if (quotedValue !== null && (!Number.isFinite(quotedValue) || quotedValue < 0)) {
  ...
  if (error) {
    ...
    showToast('Não foi possível salvar a resposta.', 'error');
    return;
  }

  const index = budgetRequestsCache.findIndex(item => String(item.id) === String(requestId));
  if (index >= 0 && data) budgetRequestsCache[index] = data;
...
window.updateBudgetRequest = updateBudgetRequest;
...
/* =========================================================
   PACOTE COMPLETO - RECURSOS ADMINISTRATIVOS
   Chat, anexos e avaliações
========================================================= */

function ensureAdminExtraPanels() {
  const anchor = getElement('budgetRequestsPanel') || getElement('budgetAlerts');
  if (!anchor || document.getElementById('adminChatPanel')) return;
  const panel = document.createElement('section');
  panel.id = 'adminChatPanel';
  panel.className = 'card feature-panel';
  panel.innerHTML = `<h3>💬 Mensagens dos clientes</h3><div id="adminChatList"><p>Nenhuma mensagem carregada.</p></div>`;
  anchor.insertAdjacentElement('afterend', panel);

  const ratingPanel = document.createElement('section');
  ratingPanel.id = 'adminRatingsPanel';
  ratingPanel.className = 'card feature-panel';
  ratingPanel.innerHTML = `<h3>⭐ Avaliações recebidas</h3><div id="adminRatingsList"><p>Nenhuma avaliação carregada.</p></div>`;
  panel.insertAdjacentElement('afterend', ratingPanel);

  const attachmentPanel = document.createElement('section');
  attachmentPanel.id = 'adminAttachmentsPanel';
  attachmentPanel.className = 'card feature-panel';
  attachmentPanel.innerHTML = `<h3>📎 Arquivos enviados pelos clientes</h3><div id="adminAttachmentsList"><p>Nenhum arquivo carregado.</p></div>`;
  ratingPanel.insertAdjacentElement('afterend', attachmentPanel);
}

async function loadAdminExtraFeatures() {
  const supabase = getSupabase();
  if (!supabase) return;
  ensureAdminExtraPanels();

  const chatBox = getElement('adminChatList');
  if (chatBox) {
    const { data: messages, error } = await supabase.from('chat_messages').select('*').order('created_at', { ascending: true });
    if (error) console.error('Mensagens administrativas:', error);
    else {
      const groups = new Map();
      (messages || []).forEach(message => {
        if (!groups.has(message.user_id)) groups.set(message.user_id, []);
        groups.get(message.user_id).push(message);
      });
      chatBox.innerHTML = groups.size ? [...groups.entries()].map(([userId, items]) => {
        const client = clientsCache.find(c => String(c.auth_user_id) === String(userId));
        const last = items[items.length - 1];
        return `<article class="admin-chat-thread"><h4>${escapeHTML(client?.name || userId)}</h4><div class="chat-messages">${items.map(item => `<div class="chat-message ${item.sender_type === 'admin' ? 'from-admin' : 'from-client'}"><strong>${item.sender_type === 'admin' ? 'Você' : 'Cliente'}</strong><p>${escapeHTML(item.message)}</p><small>${escapeHTML(formatDateTime(item.created_at))}</small></div>`).join('')}</div><form onsubmit="sendAdminChatMessage(event, '${escapeHTML(userId)}')" class="chat-form"><textarea name="message" rows="2" maxlength="4000" required placeholder="Responder ao cliente..."></textarea><button class="btn btn-primary btn-sm" type="submit">Responder</button></form></article>`;
      }).join('') : '<p>Nenhuma mensagem carregada.</p>';
    }
  }

  const ratingsBox = getElement('adminRatingsList');
  if (ratingsBox) {
    const { data: ratings, error } = await supabase.from('service_ratings').select('*').order('created_at', { ascending: false });
    if (error) console.error('Avaliações administrativas:', error);
    else ratingsBox.innerHTML = (ratings || []).length ? ratings.map(item => `<article class="rating-item"><strong>${escapeHTML(item.user_id)}</strong><p>${'★'.repeat(item.rating)}${'☆'.repeat(5-item.rating)}</p><p>${escapeHTML(item.comment || 'Sem comentário')}</p><small>${escapeHTML(formatDateTime(item.created_at))}</small></article>`).join('') : '<p>Nenhuma avaliação carregada.</p>';
  }

  const attachmentsBox = getElement('adminAttachmentsList');
  if (attachmentsBox) {
    const { data: files, error } = await supabase.from('client_attachments').select('*').order('created_at', { ascending: false });
    if (error) console.error('Anexos administrativos:', error);
    else attachmentsBox.innerHTML = (files || []).length ? (await Promise.all(files.map(async file => {
      const signed = await supabase.storage.from('client-attachments').createSignedUrl(file.storage_path, 300);
      const url = signed.data?.signedUrl;
      return `<div class="attachment-item"><span>📄 ${escapeHTML(file.file_name)}</span><small>${escapeHTML(formatDateTime(file.created_at))}</small>${url ? `<a class="btn btn-secondary btn-sm" href="${escapeHTML(url)}" target="_blank" rel="noopener">Abrir</a>` : '<span>Link indisponível</span>'}</div>`;
    }))).then(items => items.join('')) : '<p>Nenhum arquivo carregado.</p>';
  }
}

async function sendAdminChatMessage(event, userId) {
  event.preventDefault();
  const form = event.currentTarget;
  const input = form.elements.message;
  const message = input?.value.trim();
  const supabase = getSupabase();
  if (!message || !supabase || !userId) return;
  const { data: sessionData } = await supabase.auth.getSession();
  const adminId = sessionData?.session?.user?.id;
  if (!adminId) return;
  const { error } = await supabase.from('chat_messages').insert({ user_id: userId, sender_id: adminId, sender_type: 'admin', message });
  if (error) { console.error(error); showToast('Não foi possível enviar a resposta.', 'error'); return; }
  input.value = '';
  await loadAdminExtraFeatures();
  showToast('Resposta enviada.', 'success');
}

window.sendAdminChatMessage = sendAdminChatMessage;

const originalRefreshAllForFeatures = refreshAll;
refreshAll = async function (...args) {
  const result = await originalRefreshAllForFeatures(...args);
  await loadAdminExtraFeatures();
  return result;
};
window.refreshAll = refreshAll;

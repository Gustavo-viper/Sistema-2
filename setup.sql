...
-- =========================================================
-- 4. SOLICITAÇÕES DE ORÇAMENTO DOS CLIENTES
-- =========================================================
...
create table if not exists public.budget_requests (
    ...
    observations text,
    status text not null default 'pending'
        check (status in ('pending', 'reviewing', 'quoted', 'approved', 'rejected', 'cancelled')),
...
alter table public.budget_requests add column if not exists quoted_value numeric(10, 2);
alter table public.budget_requests add column if not exists admin_response text;
alter table public.budget_requests add column if not exists responded_at timestamptz;
alter table public.budget_requests add column if not exists updated_at timestamptz default now();

-- =========================================================
-- 5. FUNÇÃO DE ATUALIZAÇÃO DO updated_at
-- =========================================================
...
drop trigger if exists update_budget_requests_updated_at
...
-- =========================================================
-- 6. CRIAÇÃO AUTOMÁTICA DO PERFIL
-- =========================================================
...
-- =========================================================
-- POLÍTICAS DA TABELA BUDGET_REQUESTS
-- =========================================================

drop policy if exists "Usuário vê suas solicitações" on public.budget_requests;
drop policy if exists "Usuário cria sua solicitação" on public.budget_requests;
drop policy if exists "Administrador vê todas as solicitações" on public.budget_requests;
drop policy if exists "Administrador responde solicitações" on public.budget_requests;

create policy "Usuário vê suas solicitações"
on public.budget_requests
...
create policy "Administrador vê todas as solicitações"
on public.budget_requests
for select to authenticated
using ((select public.is_admin()));

create policy "Administrador responde solicitações"
on public.budget_requests
...
revoke all on table public.budget_requests
    from anon;
...
grant select, insert, update
    on table public.budget_requests
    to authenticated;

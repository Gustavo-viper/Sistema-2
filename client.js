...
function renderBudgetRequestCard(request) {
    ...
    const decision = request.client_decision || null;
    const decisionLabel = decision === 'accepted' ? 'Você aceitou este orçamento' : decision === 'declined' ? 'Você recusou este orçamento' : '';
    const decisionBox = decisionLabel ? `<p class="budget-decision ${decision}"><strong>${decisionLabel}</strong></p>` : '';
    const detailsButton = `<button type="button" class="btn btn-secondary btn-sm budget-details-btn" onclick="openBudgetDetails('${escapeHTML(request.id)}')">Ver detalhes</button>`;
    const signalButton = (decision === 'accepted' && Number(request.signal_value || 0) > 0 && !request.signal_paid_at)
        ? `<button type="button" class="btn btn-success btn-sm" onclick="openBudgetSignalPayment('${escapeHTML(request.id)}')">💚 Pagar sinal</button>`
        : '';
    ...
    return `
        <article class="budget-request-card">
            ...
            ${response}
            ${decisionBox}
            <div class="budget-request-actions">${detailsButton}${signalButton}</div>
...
function openBudgetDetails(requestId) {
    const request = (window.__budgetRequestsCache || []).find(item => String(item.id) === String(requestId));
    const modal = document.getElementById('budgetDetailsModal');
    const body = document.getElementById('budgetDetailsBody');
    const footer = document.getElementById('budgetDetailsFooter');
    if (!request || !modal || !body || !footer) return;

    const status = request.status || 'pending';
    const decision = request.client_decision || null;
    const canDecide = status === 'quoted' && !decision;
    const value = request.quoted_value !== null && request.quoted_value !== undefined
        ? formatMoney(request.quoted_value) : 'A definir';

    body.innerHTML = `
        <div class="budget-detail-heading">
            <span class="budget-detail-icon">📱</span>
            <div><h3>${escapeHTML(request.device_type || 'Aparelho')} ${request.device_model ? `— ${escapeHTML(request.device_model)}` : ''}</h3>
            <span class="status-badge ${escapeHTML(status)}">${escapeHTML(BUDGET_REQUEST_STATUS[status] || status)}</span></div>
        </div>
        <div class="budget-detail-grid">
            <div><small>Problema informado</small><strong>${escapeHTML(request.problem_type || 'Não informado')}</strong></div>
            <div><small>Valor do orçamento</small><strong>${value}</strong></div>
            <div><small>Data da solicitação</small><strong>${escapeHTML(formatDateTime(request.created_at))}</strong></div>
            <div><small>Observações</small><strong>${escapeHTML(request.observations || 'Nenhuma')}</strong></div>
        </div>
        ${request.admin_response ? `<div class="budget-detail-response"><small>Mensagem da assistência</small><p>${escapeHTML(request.admin_response)}</p></div>` : ''}
        ${decision === 'accepted' ? '<p class="budget-decision accepted"><strong>Você aceitou este orçamento.</strong></p>' : ''}
        ${decision === 'declined' ? '<p class="budget-decision declined"><strong>Você recusou este orçamento.</strong></p>' : ''}
    `;

    footer.innerHTML = `${canDecide ? `<button type="button" class="btn btn-danger" onclick="decideBudgetRequest('${escapeHTML(request.id)}','declined')">Recusar orçamento</button><button type="button" class="btn btn-success" onclick="decideBudgetRequest('${escapeHTML(request.id)}','accepted')">Aceitar orçamento</button>` : ''}<button type="button" class="btn btn-secondary" onclick="closeModal('budgetDetailsModal')">Fechar</button>`;
    modal.classList.add('active');
    modal.setAttribute('aria-hidden', 'false');
}

async function decideBudgetRequest(requestId, decision) {
    const client = window.supabaseClient;
    const user = window.currentUser;
    if (!client || !user || !requestId || !['accepted', 'declined'].includes(decision)) return;
    const message = decision === 'accepted' ? 'Deseja aceitar este orçamento?' : 'Deseja recusar este orçamento?';
    if (!window.confirm(message)) return;
    let data = null;
    let error = null;

    // Preferimos a função protegida no banco: ela permite alterar somente a decisão do cliente.
    const rpcResult = await client.rpc('decide_budget_request', {
    ...
    // Compatibilidade com instalações antigas que ainda não receberam a função SQL.
    if (error && ['PGRST202', '42883'].includes(String(error.code || ''))) {
        const fallback = await client.from('budget_requests')
            .update({
    ...
    if (error) {
        ...
        notify('Erro', `Não foi possível registrar sua decisão${detail}. Execute o SQL ORCAMENTO-DECISAO-SEGURA.sql no Supabase.`, 'error');
        return;
    }
    notify('Sucesso', decision === 'accepted' ? 'Orçamento aceito!' : 'Orçamento recusado.', 'success');
    closeModal('budgetDetailsModal');
    await loadMyBudgetRequests();
}

function openBudgetSignalPayment(requestId) {
    const request = (window.__budgetRequestsCache || []).find(item => String(item.id) === String(requestId));
    openPixPaymentModal(request || {}, 'signal');
}
...
async function loadMyBudgetRequests() {
    ...
    try {
        ...
        window.__budgetRequestsCache = requests;
...
function renderServiceCard(service) {
    ...
    const signalPending = !service.signal_paid_at && Number(service.signal_value || 0) > 0 && !['cancelled', 'completed'].includes(status);
    const remainingPending = !service.remaining_paid_at && Number(service.remaining_value || 0) > 0 && ['ready', 'completed'].includes(status);
...
function copyPixKey() {
    ...
    const finish = () => notify('PIX copiado', 'A chave PIX foi copiada para a área de transferência.', 'success');
    if (navigator.clipboard && window.isSecureContext) {
        navigator.clipboard.writeText(key).then(finish).catch(() => notify('Chave PIX', key, 'info'));
        return;
    }
...
function openPixPaymentModal(service = {}, paymentType = 'signal') {
    document.getElementById('pixPaymentModal')?.remove();
    const isFinal = paymentType === 'remaining';
    const rawAmount = isFinal ? service.remaining_value : service.signal_value;
    const amount = rawAmount !== null && rawAmount !== undefined && rawAmount !== ''
        ? formatMoney(rawAmount) : 'a confirmar';
    const title = isFinal ? '💙 Pagar pagamento final via PIX' : '💚 Pagar sinal via PIX';
    const description = isFinal
        ? 'Depois de pagar o valor restante, envie o comprovante pelo chat para concluirmos o atendimento.'
        : 'Depois de pagar o sinal, envie o comprovante pelo chat para confirmarmos o início do serviço.';
    const modal = document.createElement('div');
    modal.id = 'pixPaymentModal';
    modal.className = 'modal-overlay active payment-pix-modal';
    modal.setAttribute('aria-hidden', 'false');
    modal.innerHTML = `
        <div class="modal pix-payment-card" role="dialog" aria-modal="true" aria-labelledby="pixPaymentTitle">
            <div class="modal-header">
                <h2 id="pixPaymentTitle" class="modal-title">${title}</h2>
                <button type="button" class="modal-close" id="closePixPayment" aria-label="Fechar">×</button>
            </div>
            <div class="modal-body">
                <div class="pix-animation" aria-hidden="true">✓</div>
                <h3>Chave PIX da assistência</h3>
                <p>Valor a pagar: <strong>${escapeHTML(amount)}</strong></p>
                <div class="pix-key-box">
                    <span class="pix-key" id="pixPaymentKey">${PIX_SIGNAL_KEY}</span>
                    <button type="button" class="btn btn-primary btn-sm" id="copyPixPayment">Copiar chave</button>
                </div>
                <p class="muted">${description}</p>
                <p class="pix-warning">Confira o destinatário antes de confirmar a transferência.</p>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" id="closePixPaymentFooter">Fechar</button>
            </div>
        </div>`;
    document.body.appendChild(modal);
    const close = () => modal.remove();
    modal.querySelector('#closePixPayment')?.addEventListener('click', close);
    modal.querySelector('#closePixPaymentFooter')?.addEventListener('click', close);
    modal.querySelector('#copyPixPayment')?.addEventListener('click', copyPixKey);
...
async function paySignal(serviceId) {
    ...
    try {
        const { data: service, error } = await window.supabaseClient
            .from('services').select('id, signal_value, remaining_value, total_value, device_type, device_model')
            .eq('id', serviceId).maybeSingle();
        if (error) throw error;
        openPixPaymentModal(service || {}, 'signal');
    } catch (error) {
        console.error('Erro ao carregar sinal:', error);
        openPixPaymentModal({}, 'signal');
    }
}
...
async function payRemaining(serviceId) {
    ...
    try {
        const { data: service, error } = await window.supabaseClient
            ...
            .eq('id', serviceId).maybeSingle();
        if (error) throw error;
        openPixPaymentModal(service || {}, 'remaining');
    } catch (error) {
        console.error('Erro ao carregar pagamento final:', error);
        openPixPaymentModal({}, 'remaining');
    }
}
...
async function rateService(serviceId) {
    ...
    modal.setAttribute('aria-hidden', 'false');
    modal.innerHTML = `
        <div class="modal" role="dialog" aria-modal="true" aria-labelledby="ratingTitle">
            <div class="modal-header">
                <h2 id="ratingTitle" class="modal-title">⭐ Avaliar atendimento</h2>
                <button type="button" class="modal-close" id="closeRating" aria-label="Fechar">×</button>
            </div>
            <form id="ratingForm" class="modal-body">
                <label class="form-label" for="ratingValue">Sua nota</label>
                <select id="ratingValue" class="form-select" required>
                ...
                <label class="form-label" for="ratingComment">Comentário (opcional)</label>
                <textarea id="ratingComment" class="form-textarea" maxlength="1000" rows="4" placeholder="Conte como foi seu atendimento..."></textarea>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" id="cancelRating">Cancelar</button>
                    <button type="submit" class="btn btn-primary" id="submitRating">Enviar avaliação</button>
                </div>
            </form>
        </div>`;
    document.body.appendChild(modal);
    const close = () => modal.remove();
    modal.querySelector('#closeRating')?.addEventListener('click', close);
    modal.querySelector('#cancelRating')?.addEventListener('click', close);
    modal.addEventListener('click', event => { if (event.target === modal) close(); });
    modal.querySelector('#ratingForm')?.addEventListener('submit', async event => {
        event.preventDefault();
        const rating = Number(modal.querySelector('#ratingValue').value);
        const comment = modal.querySelector('#ratingComment').value.trim();
        const button = modal.querySelector('#submitRating');
        if (!rating) return notify('Atenção', 'Selecione uma nota.', 'warning');
        button.disabled = true;
        button.textContent = 'Salvando...';
        const { error } = await window.supabaseClient.from('service_ratings').upsert({
        ...
        if (error) {
            console.error('Avaliação:', error);
            notify('Erro', 'Não foi possível salvar a avaliação. Execute o SQL das avaliações no Supabase.', 'error');
            return;
        }
        notify('Obrigado!', 'Sua avaliação foi registrada com sucesso.', 'success');
        close();
...
window.decideBudgetRequest = decideBudgetRequest;
window.openBudgetDetails = openBudgetDetails;
window.openBudgetSignalPayment = openBudgetSignalPayment;

...
function renderBudgetRequestCard(request) {
    ...
    const decisionButtons = status === 'quoted' && !decision
        ? `<div class="budget-decision-actions"><button type="button" class="btn btn-success btn-sm" onclick="decideBudgetRequest('${escapeHTML(request.id)}', 'accepted')">Aceitar orçamento</button><button type="button" class="btn btn-danger btn-sm" onclick="decideBudgetRequest('${escapeHTML(request.id)}', 'declined')">Recusar orçamento</button></div>`
        : '';
...
async function decideBudgetRequest(requestId, decision) {
    ...
    const { data, error } = await client.from('budget_requests')
        .update({ client_decision: decision, client_decision_at: new Date().toISOString(), status: decision === 'accepted' ? 'approved' : 'rejected' })
        .eq('id', requestId).eq('user_id', user.id).select().single();
    if (error) {
        console.error('Erro ao registrar decisão do orçamento:', error);
        notify('Erro', 'Não foi possível registrar sua decisão. Execute o SQL de atualização.', 'error');
        return;
    }
